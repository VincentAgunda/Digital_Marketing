...
// Variants for the heading animation (staggering lines)
const headingContainerVariants = {
  hidden: {},
  visible: { transition: { staggerChildren: 0.2, delayChildren: 0.1 } },
};

// Variants for each line within the heading
const lineVariants = {
  hidden: { opacity: 0, y: 20 },
  visible: { opacity: 1, y: 0, transition: { duration: 0.5, ease: "easeOut" } },
};

// Variants for text/elements animating in on scroll
const fadeInOnScrollVariants = {
    hidden: { opacity: 0, y: 20 },
    visible: { opacity: 1, y: 0, transition: { duration: 0.6 } }
};

// Variants for the team member cards inside the popup (matched to modal spring)
const teamCardPopupAnimation = {
  hidden: { opacity: 0, y: 20 },
  visible: {
    opacity: 1,
    y: 0,
...
// Variants for the Design Principle cards container (for staggering)
const principlesContainerVariants = {
    hidden: {},
    visible: { transition: { staggerChildren: 0.15 } }
};

// Variants for individual Design Principle cards
...
// --- Helper Component for Optimized Icon Pulse ---
...
const PulsingIcon = ({ children, index }) => {
  ...
  useEffect(() => {
    if (isInView) {
      controls.start({
        scale: [1, 1.1, 1],
        transition: { duration: 2, repeat: Infinity, repeatType: "loop", ease: "easeInOut", delay: index * 0.3 }
      });
    } else {
      controls.stop();
  ...
  return (
    <motion.div ref={ref} className="text-2xl mb-3" animate={controls} initial={{ scale: 1 }}>
      {children}
    </motion.div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-gray-100 font-sans overflow-x-hidden">
      ...
      <section className="container mx-auto px-6 py-12 md:py-20 flex flex-col md:flex-row items-center gap-8 md:gap-12">
        <div className="flex-1">
          <motion.h1
            ...
            animate="visible"
          >
            <motion.span className="block" variants={lineVariants}> <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Crafting</span> digital </motion.span>
            <motion.span className="block" variants={lineVariants}> <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-500">experiences</span> with </motion.span>
            <motion.span className="block" variants={lineVariants}> <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-500">purpose</span> </motion.span>
          </motion.h1>
        ...
        >
          <img src="camera1.webp" alt="Digital interface showing abstract design" className="w-full h-full object-cover" loading="lazy" />
        </motion.div>
      </section>
      ...
      <AnimatePresence>
        {showTeam && (
          <motion.div
            role="dialog" aria-modal="true" aria-labelledby="team-popup-title"
              ...
              >
                {teamMembers.map((member) => (
                  <motion.div
                    ...
                    variants={teamCardPopupAnimation}
                  >
                    ...
                    <div className="p-4 flex flex-col flex-grow">
                      ...
                      <div className="flex space-x-2 mt-auto">
                         {/* Ensure SVGs are correctly placed within links */}
                         <a href={member.linkedin} target="_blank" rel="noopener noreferrer" aria-label={`${member.name}'s LinkedIn`} className="text-gray-400 hover:text-cyan-400 transition-colors">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M16 8a6 6 0 016 6v7h-4v-7a2 2 0 00-2-2 2 2 0 00-2 2v7h-4v-7a6 6 0 016-6zM2 9h4v12H2z"/><circle cx="4" cy="4" r="2"/></svg>
                         </a>
                         <a href={member.twitter} target="_blank" rel="noopener noreferrer" aria-label={`${member.name}'s Twitter`} className="text-gray-400 hover:text-cyan-400 transition-colors">
      ...
      <section className="container mx-auto px-6 py-16 text-center">
        <motion.div
            ...
            transition={{ duration: 0.6 }}
        >
            {/* Corrected: Removed ellipsis from className */}
            ...
            {/* Corrected: Removed ellipsis from className and fixed whileHover prop */}
            <motion.button
                ...
                whileHover={{ scale: 1.05, transition: { duration: 0.2 } }} // Corrected this line
                whileTap={{ scale: 0.95 }}
            >
                Start a project
            </motion.button>
